...
**Actively updated.** The History category is the first available skill. More categories will be added, and the existing ones will continue to be refined.
...
`john-locke-history-essay` is a collaborative writing workflow for the History category.
...
└── references/
    ├── winning-patterns.md   # 8 judgment patterns distilled from placed essays
    └── anti-patterns.md      # 12 failure modes for scanning a draft
```

`SKILL.md` is the skeleton; the reference files are the substance.
...
- the full list of History questions for the year, exactly as published
- the word limit, and what is excluded from the count
- submission requirements: filename format, citation and notes rules, anonymity, academic referee requirement, and the **AI‑use policy** for the year（Please do no **delete or modify some rules**）
...
Use it in any product that supports Agent Skills.
 Start a conversation about your John Locke History essay, paste in the current year’s rules when prompted, and proceed through the checkpoints one by one.
...
**Not validated by real judges.**  
 Essays produced with this skill have never been evaluated by the John Locke Institute or by any human marker.
 ...
 **This does not indicate anything real.**  
 Model preferences do not predict human judgment. Treat the skill as a drafting and self‑critique tool, not a path to winning.

**Small and dated sample.**  
 The techniques are reverse‑engineered from a small number of placed essays from the 2019, 2020, 2021, and 2023 cycles.
...
**You are responsible for the final essay.**  
 The skill flags unverified claims, but you must confirm every source, fact, and quotation yourself, stay within the word limit, and follow the submission rules.

**This skill has not been tested across all models.**  
 Different models may behave differently; always double‑check the output.
...
- Expand beyond History into categories such as Economics and Law
- Continue refining the History technique library as more placed essays become available
